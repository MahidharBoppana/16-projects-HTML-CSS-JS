...
async function searchUser() {
  ...
  try {
    ...
    if (!reponse.ok) throw new Error("User not found");
...
async function fetchRepositories(reposUrl) {
  ...
  try {
    const reponse = await fetch(reposUrl + "?per_page=6");
...
function displayUserData(user) {
  ...
  bioElement.textContent = user.bio || "No bio availble";
...
searchInput.value = "MahidharBoppana";
searchUser();
